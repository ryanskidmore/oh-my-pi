...
| `CLOUDFLARE_ACCOUNT_ID`         | Cloudflare account routing (AI Gateway and Workers AI) | Using an environment-supplied token for either provider  | Not required when the account ID was stored by `/login`                                             |
...
| `CLOUDFLARE_WORKERS_AI_API_KEY` | Cloudflare Workers AI auth                       | Using `cloudflare-workers-ai` provider                         | API token with Workers AI Read + Edit; `/login` stores it with the account ID                       |
| `CLOUDFLARE_API_TOKEN`          | Cloudflare Workers AI auth                       | Fallback when `CLOUDFLARE_WORKERS_AI_API_KEY` is unset          | Standard Cloudflare API token variable                                                               |
