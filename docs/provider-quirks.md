...
### Special casings
- **Prompt-Cache Session Header**: `x-session-affinity` is sent via the `prompt-cache-session-header` wire axis declared in `packages/catalog/src/compat/rules/providers/cloudflare-workers-ai.kdl`, applied by `resolveOpenAIRequestSetup` (`packages/ai/src/providers/openai-shared.ts`). Without it, `cached_tokens` is always 0.
...
- **Developer Role & Store Disabled**: `supports-developer-role #false` and `supports-store #false` in the provider KDL; both fields are accepted (HTTP 200), but this endpoint accepts every unknown field it is handed, so acceptance is not evidence the semantics are honored — both stay off and system prompts keep riding the `system` role.
...
- **Ladderless Reasoning Fallback**: Rows that report `reasoning: true` but publish no `reasoning.supported_efforts` vocabulary at all get the ladder `low`/`medium`/`high` from a provider-root `thinking-efforts` rule in `packages/catalog/src/compat/rules/providers/cloudflare-workers-ai.kdl`, because the endpoint rejects `reasoning_effort` `minimal`/`xhigh` with an HTTP 400 (AiError code 8001) on such rows (verified live on `@cf/nvidia/nemotron-3-120b-a12b` and `@cf/zai-org/glm-4.7-flash`). A discovered ladder always wins over this fallback — `resolveThinkingPolicy` only reaches it when the discovery mapper left `thinking` unset.
...
- **Plain-String Message Content**: `requires-string-message-content #true` in the provider KDL makes `convertMessages` (`packages/ai/src/providers/openai-completions.ts`) collapse every text-only `messages[].content` array into one string joined by `\n`. Each request is validated against the served model's own JSON Schema, and five rows (`@cf/openai/gpt-oss-120b`, `@cf/openai/gpt-oss-20b`, `@cf/meta/llama-3.3-70b-instruct-fp8-fast`, `@cf/ibm-granite/granite-4.0-h-micro`, `@cf/qwen/qwen3-30b-a3b-fp8`) type `content` as a string *or one* content part: any message whose array holds two or more parts fails with HTTP 400 `AiError: Bad input: Error: oneOf at '/' not met … Type mismatch of '/messages/N/content'` (code 5006), whatever the sibling messages carry — so promoting every message to an array is not a workaround. Because omp emits one text part per context block, an ordinary "environment preamble + question" user turn tripped it. A plain string is accepted by all 18 rows. Content carrying an image keeps its array (an image has no string encoding); only multimodal rows receive one, and they accept a multi-part array beside a string system prompt.
...
- **Output Cap Leaves Prompt Room**: `max_output_length` is ignored because it echoes `context_length` rather than reporting a real cap, so `mapCloudflareWorkersAiModel` (`packages/catalog/src/provider-models/openai-compat.ts`) seeds `maxTokens = min(OPENAI_COMPAT_DISCOVERY_DEFAULT_MAX_TOKENS, floor(contextWindow / 4))`, falling back to the 32,768 default when the row publishes no context window. The quarter matters because `always-send-max-tokens #true` puts this value on **every** request and the endpoint validates `prompt + max_tokens <= context_length`: clamping to the context window alone made the cap equal the whole window on rows at or below 32,768, so every request failed with HTTP 400 "This model's maximum context length is 24000 tokens. However, you requested 24000 output tokens and your prompt contains …" (omp has no request-time context-aware clamp — `clampOutputToModelMax` only clamps to `model.maxTokens`). Rows at 131,072 context and above are unchanged at 32,768; 128,000 → 32,000; 32,768 → 8,192; 24,000 → 6,000. Per-model corrections belong in a KDL `limits-patch`.
- **Known Limitation — Very Small Context Windows**: Models with very small context windows (for example `@cf/meta/llama-3.3-70b-instruct-fp8-fast` at 24,000 tokens) cannot hold omp's default system prompt plus tool definitions (~20,000 tokens measured), so they fail with a context-length HTTP 400 regardless of the output cap; they remain usable with a reduced tool set or a custom system prompt.
- **Paid-Only Models Not Filtered**: The `format=openrouter` discovery projection omits the native `require_workers_paid` flag, so a free-plan account still sees and can select paid-only models — including the declared `default-model`, `@cf/moonshotai/kimi-k2.7-code` — and only discovers the restriction at request time, as an HTTP 403 (Cloudflare's "model requires Workers Paid" error, AiError code 5035). Switching discovery to the native format to filter these rows would cost the whole pricing/modalities/reasoning mapping reuse the OpenRouter projection gives for free, so this is accepted rather than worked around.
