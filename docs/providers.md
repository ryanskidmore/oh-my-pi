...
`/login cloudflare-workers-ai` prompts for the API token and account ID and stores both together. The token needs **Workers AI: Read + Edit**; find both at Workers AI → Use REST API → Create a Workers AI API Token in the Cloudflare dashboard (the account ID is on the same screen). The model list is discovered live from the account's Workers AI models-search endpoint, so only tool-calling text-generation models are offered and no `models.yml` base URL override is needed. Unlike `cloudflare-ai-gateway`, which proxies requests through the gateway with a gateway-scoped token, `cloudflare-workers-ai` calls the account's Workers AI API directly with a Workers AI-scoped token and its own env vars.
